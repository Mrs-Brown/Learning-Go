// Capstone for book, Life On Mars
package main
...
const (
	...
	//Length which message can be transmitted from rover to Earth
	receiveTimePerDay = 2 * time.Second
)

// earthReceiver receives messages sent from Mars at limited connectivity
...
func startDriver(name string, grid *MarsGrid, marsToEarth chan []Message) *RoverDriver {
	var o *Occupier
	//Try a random point, continue until find one not occupied
	for o == nil {
...
// Radio is a radio transmitter that sends msgs to Earth
type Radio struct {
	fromRover chan Message
}

// SendToEarth sends a message to Earth immediately after buffered
func (r *Radio) SendToEarth(m Message) {
	r.fromRover <- m
}

// NewRadio returns a new Radio instance that sends msgs on the toEarth channel
...
// run buffers mesages sent by a rover until they can be sent to Earth
...
// RoverDriver drives a rover around Mars
...
// drive is a goroutine that drives the rover
...
// SensorData holds info about whats in a point in the grid
...
// NewMarsGrid returns new MarsGrid of the given size
...
// occupied or is outside the grid. Otherwise it returns a value that can be used to move to
...
// MoveTo moves the occupier to a different cell in the grid, reports whether the move was
// successful, may fail if outside grid or if cell is occupied. If fails, remains in same spot

// Use previous lessons to determine distance to and from each rover on Mars and locations on Earth
package main
...
// coordinates in degrees, min, sec, in a N/S/E/W hemisphere
...
// newLocation from latitude and longitude d/m/s coordinates
...
func main() {
	...
	fmt.Printf("Spirt to Opportunity %.2f km\n", mars.distance(spirit, opportunity))
	...
	fmt.Printf("Edmonton to Ottawa %.2f km\n", earth.distance(edmonton, ottawa))
	fmt.Printf("Mount Sharp to Olympus %.2f km\n", mars.distance(mountSharp, olympusMons))
}
